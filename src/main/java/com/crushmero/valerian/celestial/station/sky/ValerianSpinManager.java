package com.crushmero.valerian.celestial.station.sky;
...
public class ValerianSpinManager {
	...
	private AbstractWorldProvider worldProvider;
	private boolean clientSide = true;

	public ValerianSpinManager(AbstractWorldProvider provider) {
	...
	public boolean refresh(BlockPos baseBlock, boolean placingThruster) {
		if (this.oneSSBlock == null || this.worldProvider.world.getBlockState(this.oneSSBlock).getBlock().isAir(
				this.worldProvider.world.getBlockState(this.oneSSBlock), this.worldProvider.world, this.oneSSBlock)) {
			if (baseBlock != null) {
			...
			for (BlockVec3 vec : currentLayer) {
				...
				for (int side = 0; side < 6; side++) {
					...
					if (sideVec != null && !this.checked.contains(sideVec)) {
						...
						if (b != null && !b.isAir(state, this.worldProvider.world, sideVec.toBlockPos())
								&& !(b instanceof BlockDynamicLiquid)) {
							...
							if (b instanceof BlockSpinThruster && !RedstoneUtil
									.isBlockReceivingRedstone(this.worldProvider.world, sideVec.toBlockPos())) {
		...
		if (placingThruster && !this.checked.contains(new BlockVec3(baseBlock))) {
			if (foundThrusters.size() > 0) {
				if (ConfigManagerCore.enableDebug) {
					GCLog.info("Thruster placed on wrong part of space station: base at " + this.oneSSBlock
							+ " - baseBlock was " + baseBlock + " - found " + foundThrusters.size());
				}
				return false;
			}
		...
		GalacticraftCore.packetPipeline.sendToDimension(
				new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_DATA,
						GCCoreUtil.getDimensionID(this.worldProvider), objList),
		...
		GalacticraftCore.packetPipeline.sendToDimension(
				new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_BOX,
						GCCoreUtil.getDimensionID(this.worldProvider), objList),
				GCCoreUtil.getDimensionID(this.worldProvider));
		...
		if (!this.clientSide) {
			...
			if (this.doSpinning) {
				...
				if (updateNeeded) {
					...
					GalacticraftCore.packetPipeline.sendToDimension(
							new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_SPIN,
									GCCoreUtil.getDimensionID(this.worldProvider), objList),
							GCCoreUtil.getDimensionID(this.worldProvider));
				}
			}
			...
			for (Entity e : this.loadedEntities) {
				if ((e instanceof EntityItem || e instanceof EntityLivingBase && !(e instanceof EntityPlayer)
						|| e instanceof EntityTNTPrimed || e instanceof EntityFallingBlock) && !e.onGround) {
					AxisAlignedBB entityBoundingBox = e.getEntityBoundingBox();
					boolean outsideStation = entityBoundingBox.maxX < this.ssBoundsMinX
							|| entityBoundingBox.minX > this.ssBoundsMaxX || entityBoundingBox.maxY < this.ssBoundsMinY
							|| entityBoundingBox.minY > this.ssBoundsMaxY || entityBoundingBox.maxZ < this.ssBoundsMinZ
							|| entityBoundingBox.minZ > this.ssBoundsMaxZ;

					if (outsideStation
							|| FreefallHandler.testEntityFreefall(this.worldProvider.world, entityBoundingBox)) {
						if (this.doSpinning) {
	...
	public boolean updatePlayerForSpin(EntityPlayerSP p, float partial) {
		...
		if (this.doSpinning && angleDelta != 0F) {
			...
			if (p.world.getCollisionBoxes(p, p.getEntityBoundingBox()).size() == 0) {
				...
				int collisions = 0;
				do {
					List<AxisAlignedBB> list = p.world.getCollisionBoxes(p,
							p.getEntityBoundingBox().expand(offsetX, 0.0D, offsetZ));
	...
	public void sendPackets(EntityPlayerMP player) {
		...
		if (player == null) {
			...
			GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_SPIN,
					GCCoreUtil.getDimensionID(player.world), objList), player);
		}
		...
		if (player == null) {
			...
			GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_DATA,
					GCCoreUtil.getDimensionID(player.world), objList), player);
		}
		...
		if (player == null) {
			...
			GalacticraftCore.packetPipeline.sendTo(new PacketSimple(PacketSimple.EnumSimplePacket.C_UPDATE_STATION_BOX,
					GCCoreUtil.getDimensionID(player.world), objList), player);
		}
	}
...
//            this.oneSSBlock = BlockVec3.readFromNBT(oneBlock);
			this.oneSSBlock = new BlockPos(oneBlock.getInteger("x"), oneBlock.getInteger("y"),
					oneBlock.getInteger("z"));
		} else {
