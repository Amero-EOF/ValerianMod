...
public abstract class ChunkProviderEmpty extends ChunkGeneratorOverworld {
	...
	public ChunkProviderEmpty(World par1World, long seed, boolean mapFeaturesEnabled) {
	...
	public void generateTerrain(int chunkX, int chunkZ, ChunkPrimer primer) {
		...
		for (int x = 0; x < ChunkProviderEmpty.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderEmpty.CHUNK_SIZE_Z; z++) {
				...
				final double featureFilter = this.noiseGen5.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderEmpty.MAIN_FEATURE_FILTER_MOD;
				final double largeFilter = this.noiseGen6.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderEmpty.LARGE_FEATURE_FILTER_MOD;
				final double smallFilter = this.noiseGen7.getNoise(chunkX * 16 + x, chunkZ * 16 + z) * ChunkProviderEmpty.SMALL_FEATURE_FILTER_MOD - 0.5;
				...
				for (int y = 0; y < ChunkProviderEmpty.CHUNK_SIZE_Y; y++) {
	...
	public void replaceBiomeBlocks(int x, int z, ChunkPrimer primer, Biome[] biomesIn) {
		...
		for (int var8 = 0; var8 < 16; ++var8) {
			for (int var9 = 0; var9 < 16; ++var9) {
				...
				for (int var16 = ChunkProviderEmpty.CHUNK_SIZE_Y - 1; var16 >= 0; --var16) {
